...
const ProductListPage = () => {
  ...
  const load = useCallback(async () => { try { const query = Object.fromEntries(Object.entries({ ...filters, limit: 10 }).filter(([, value]) => value !== '')); const [result, categories, suppliers] = await Promise.all([productsApi.list(query), categoriesApi.list(), suppliersApi.list()]); setProducts(result.data || []); setPagination(result.pagination || { page: 1, pages: 1 }); setOptions({ categories: categories.data || [], suppliers: suppliers.data || [] }); setStatus({ loading: false, error: '' }); } catch (error) { setStatus({ loading: false, error: error.response?.data?.error?.message || 'Unable to load products.' }); } }, [filters]);
  useEffect(() => { load(); }, [load]);
